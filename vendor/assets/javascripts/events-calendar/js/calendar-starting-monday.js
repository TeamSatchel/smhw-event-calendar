...
$.fn.calendar = function (options) {
    ...
    return this.each(function () {
      ...
      var curr = new Date;
      var curr_week_firstday = new Date(curr.setDate(curr.getDate() - curr.getDay()-6)).getDate();
      var curr_week_lastday = new Date(curr.setDate(curr.getDate() - curr.getDay()+7)).getDate();
      //alert(curr_week_firstday + ' ' + curr_week_lastday);


        while (running_day_count <= total_days_in_month) {

          //if (running_day_count < curr_week_firstday || running_day_count > curr_week_lastday) console.log(running_day_count );
          // output only current week
          if (running_day_count < curr_week_firstday || running_day_count > curr_week_lastday) {
            tbody += '</tr> <tr>';
            running_day.setDate(date.getDate() + 1);
            running_day_count += 1; continue;
          };
