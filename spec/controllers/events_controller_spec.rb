  ...
  describe 'POST #create' do
    ...
    context 'when event parameters are correct' do
      ...
      it 'returns a created event' do
        ...
        expect(JSON.parse(response.body)['title']).to include(new_event_params[:title])
        expect(JSON.parse(response.body)['starts_at']).to include(new_event_params[:starts_at])
        expect(JSON.parse(response.body)['ends_at']).to include(new_event_params[:ends_at])
      end
    end
  end
